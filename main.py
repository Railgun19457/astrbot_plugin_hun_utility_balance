...
class HNUUtilityBalancePlugin(Star):
	...
	async def _query_with_client(
		...
		if user_data.get("statusCode") != "200":
			return {}, user_data.get("message") or "用户信息查询失败，请确认 openid 有效。"
	...
	def _load_reminders(self) -> list[Reminder]:
		...
		for index, item in enumerate(raw_reminders):
			...
			template_key = str(
				item.get(TEMPLATE_KEY_FIELD) or item.get(LEGACY_TEMPLATE_KEY_FIELD) or ""
			).strip()
	...
	def _format_alert_message(
		...
		location = " ".join(
			part
			for part in [str(result.get("building") or ""), str(result.get("room") or "")]
			if part
		)
